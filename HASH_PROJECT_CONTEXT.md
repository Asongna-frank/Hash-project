...
| Notifications | Twilio / Termii | Future — SMS for choronko track |
...
│   ├── patient.py             # Patient model — M2 adds new columns
│   ├── pregnancy.py           # NEW: Pregnancy model
│   └── risk_assessment.py     # NEW: RiskAssessment model
├── schemas/
│   ├── hospital.py            # ✅ done
│   ├── patient.py             # updated for M2 fields
│   ├── pregnancy.py           # NEW
...
│   ├── auth.py                # ✅ done
│   ├── hospitals.py           # ✅ done
│   ├── patients.py            # NEW: patient profile + risk override endpoints
│   └── chat.py                # NEW: POST /chat/message (loss detection entry point)
├── services/
│   ├── llm_service.py         # NEW: Groq/Bedrock abstraction
│   ├── prompts.py             # NEW: all LLM prompt strings + keyword lists
│   ├── loss_detection.py      # NEW: keyword layer + LLM confirmation logic
│   └── risk_scoring.py        # NEW: compute_risk_level() from questionnaire answers
└── utils/
    ├── auth.py                # ✅ done
...
*Last updated: Sprint 2 — M2 Patient & Pregnancy Profile*
*Status: M1 complete. M2 in progress.*
*Source: SRS HASH MVP v1.2 + product clarifications*
